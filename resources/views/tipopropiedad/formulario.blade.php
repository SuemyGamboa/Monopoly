...
@section('titulo')
Formulario de tipo de propiedad
@endsection
...
<div class="container">
    ...
    <br>
    <br>
        <h2>Formulario del Tipo de propiedad</h2>
        <!--Necesita de esto para las imagenes: enctype="multipart/form-data"-->
        <form action="{{ action([App\Http\Controllers\TipopropiedadController:: class, 'save']) }}" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            <input type="hidden" name="id" value="{{ $tipopropiedad->id }}">
            <div class="form-group">
                <label for="nombre">Nombre</label>
                <input type="text" class="form-control" name="nombre" value="{{ $tipopropiedad->nombre }}">
            </div>
